...
class Store {
    ...
    async getCategory(req, res) {
        const {name} = req.body
        const Category = await post.query({text: 'select * from m_item_categories'+ (name ? ' where name = $1;' : ';'), values: (name ? [name] : [])})
        res.send(Category.rows)
    }

    async updateCategory(req, res) {
        const {id, name} = req.body
        const Category = await post.query({text: 'update m_item_categories set name = $2 where id = $1 returning *;', values: [id, name]})
        res.send(Category)
    }
    ...
    async getItem(req, res) {
        ...
        let i = 0
        const q1 = (name ? 'a.name = $'+ (++i) : '') + (category ? (i!=0?' and ':'') + 'b.name = $'+ (++i) : '')
    ...
    async moveItems(req, res) {
        ...
        //const Query = {text: 'select a.count from m_item_names where '+(item_id ? 'id':'name')+' = $1;', values: [(item_id ? parseInt(item_id) : item_name)]}
